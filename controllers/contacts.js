...
const postNew = async (req, res) => {
    ...
    if (response.acknowledged) {
        ...
        res.status(500).json(response.error || 'Something bad happened and the contact was not created.');
    }
};
...
const putUpdate = async (req, res) => {
    ...
    if (response.modifiedCount > 0) {
        ...
        res.status(500).json(response.error || 'This contact was not updated, there was an error.');
    }
};
